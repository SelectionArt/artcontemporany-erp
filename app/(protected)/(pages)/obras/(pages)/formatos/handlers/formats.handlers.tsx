...
import type {
  ...
  SubmitHandlerProps,
} from "./types/formats.handlers.types";
...
const submitHandlerCreate = async ({
  ...
  try {
    const { format, error, success } = await createFormat({ values });
    ...
    if (success && format) {
      setData((prev) =>
        [...prev, format].sort((a, b) => a.name.localeCompare(b.name)),
      );
      form.reset();
      setOpenDialog(false);
    ...
    console.error(error);
    toast.error("Error al crear el formato. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerEdit = async ({
  ...
  try {
    const { format, error, success } = await updateFormat({
      id: selectedRow.id,
      values,
    });
    ...
    if (success && format) {
      setData((prev) =>
        prev
          .map((item) => (item.id === format.id ? format : item))
    ...
    console.error(error);
    toast.error(
      "Error al actualizar el formato. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const submitHandlerDelete = async ({
  ...
  try {
    const { success, error } = await deleteFormat({ id: selectedRow.id });
    ...
    console.error(error);
    toast.error("Error al eliminar el formato. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultipleFormats({
    ...
    console.error(error);
    toast.error(
      "Error al eliminar los formatos. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const FormatsHandlers = ({
  ...
  setSelectedRow,
  setSelectedRows,
}: FormatsHandlersProps): FormatsHandlersReturn => {
  return {
