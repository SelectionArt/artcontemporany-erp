...
import type {
  ...
  SubmitHandlerProps,
} from "./types/styles.handlers.types";
...
const submitHandlerCreate = async ({
  ...
  try {
    const { style, error, success } = await createStyle({ values });
    ...
    if (success && style) {
      setData((prev) =>
        [...prev, style].sort((a, b) => a.name.localeCompare(b.name)),
      );
      form.reset();
      setOpenDialog(false);
    ...
    console.error(error);
    toast.error("Error al crear el estilo. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerEdit = async ({
  ...
  try {
    const { style, error, success } = await updateStyle({
      id: selectedRow.id,
      values,
    });
    ...
    if (success && style) {
      setData((prev) =>
        prev
          .map((item) => (item.id === style.id ? style : item))
    ...
    console.error(error);
    toast.error("Error al actualizar el estilo. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDelete = async ({
  ...
  try {
    const { success, error } = await deleteStyle({ id: selectedRow.id });
    ...
    console.error(error);
    toast.error("Error al eliminar el estilo. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultipleStyles({
    ...
    console.error(error);
    toast.error("Error al eliminar los estilos. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const StylesHandlers = ({
  ...
  setSelectedRow,
  setSelectedRows,
}: StylesHandlersProps): StylesHandlersReturn => {
  return {
