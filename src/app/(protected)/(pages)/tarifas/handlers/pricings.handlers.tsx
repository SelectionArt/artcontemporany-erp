...
const submitHandlerCreate = async ({
  ...
  try {
    const { pricing, error, success } = await createPricing({ values });
    ...
    if (success && pricing) {
      setData((prev) =>
        [...prev, pricing].sort((a, b) => a.name.localeCompare(b.name)),
      );
      form.reset();
      setOpenDialog(false);
...
const submitHandlerDeleteMultiple = async ({
  ...
  try {
    ...
    console.error(error);
    toast.error("Error al eliminar las tarifas. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
