...
const statusChangeHandler = async ({
  ...
  setData,
  setLoading,
}: StatusChangeHandlerProps): Promise<void> => {
  setLoading(true);

  try {
    const { error, success } = await updateStatus({ id, newStatus });
    ...
    console.error(error);
    toast.error("Error al actualizar el estado. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const BudgetsHandlers = ({
  ...
  return {
    ...
    handleStatusChange: ({ id, newStatus }) =>
      statusChangeHandler({ id, newStatus, setData, setLoading }),
  };
};
