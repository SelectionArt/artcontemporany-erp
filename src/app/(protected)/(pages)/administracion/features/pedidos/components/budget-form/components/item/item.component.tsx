...
const Item = ({
  ...
  const [searchValueArtwork, setSearchValueArtwork] = useState("");
  ...
  useEffect(() => {
    const selectedArtwork = artworks.find((a) => a.id === items?.artworkId);
    const selectedFrame = frames.find((f) => f.id === items?.frameId);
    if (selectedFrame) {
      const label = `${selectedFrame.reference} ${selectedFrame.name}`;
      if (searchValueFrame !== label) {
        setSearchValueFrame(label);
      }
    } else if (searchValueFrame !== "") {
      setSearchValueFrame("");
    }
    if (selectedArtwork) {
      const label = `${selectedArtwork.referenceNumber}${
        selectedArtwork.referenceCode ? `-${selectedArtwork.referenceCode}` : ""
      }${selectedArtwork.title ? ` · ${selectedArtwork.title}` : ""}`;
      if (searchValueArtwork !== label) {
        setSearchValueArtwork(label);
      }
    } else if (searchValueArtwork !== "") {
      setSearchValueArtwork("");
    }
  ...
  const artworksValues = (selectedId?: string) => {
    ...
    if (selectedArtwork) {
      const label = `${selectedArtwork.referenceNumber}${selectedArtwork.referenceCode ? `-${selectedArtwork.referenceCode}` : ""}${selectedArtwork.title ? ` · ${selectedArtwork.title}` : ""}`;
      return [
        {
            ...
            <FormItem className="min-w-[160px]">
              <FormControl>
                <AutoComplete
                  ...
                  items={artworksValues(field.value)}
                  onSearchValueChange={setSearchValueArtwork}
                  onSelectedValueChange={field.onChange}
                  placeholder="Referencia obra"
                  searchValue={searchValueArtwork}
            ...
            <FormItem className="min-w-[160px]">
              <FormControl>
                <AutoComplete
                  ...
                  items={framesValues(field.value)}
