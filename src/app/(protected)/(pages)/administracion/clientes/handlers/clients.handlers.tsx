...
import type {
  ...
  SubmitHandlerProps,
} from "./types/clients.handlers.types";
...
const submitHandlerCreate = async ({
  ...
  try {
    const { client, error, success } = await createClient({ values });
...
const submitHandlerEdit = async ({
  ...
  try {
    const { client, error, success } = await updateClient({
      id: selectedRow.id,
      values,
    });
...
const submitHandlerDelete = async ({
  ...
  try {
    const { success, error } = await deleteClient({ id: selectedRow.id });
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultipleClients({
...
const ClientsHandlers = ({
  ...
  setSelectedRow,
  setSelectedRows,
}: ClientsHandlersProps): ClientsHandlersReturn => {
  return {
