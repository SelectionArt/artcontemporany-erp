...
import type {
  ...
  SubmitHandlerProps,
} from "./types/materials.handlers.types";
...
const submitHandlerCreate = async ({
  ...
  try {
    const { material, error, success } = await createMaterial({ values });
    ...
    console.error(error);
    toast.error("Error al crear el material. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerEdit = async ({
  ...
  try {
    const { material, error, success } = await updateMaterial({
      id: selectedRow.id,
      values,
    });
    ...
    console.error(error);
    toast.error(
      "Error al actualizar el material. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const submitHandlerDelete = async ({
  ...
  try {
    const { success, error } = await deleteMaterial({ id: selectedRow.id });
    ...
    console.error(error);
    toast.error("Error al eliminar el material. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultipleMaterials({
    ...
    console.error(error);
    toast.error(
      "Error al eliminar los materiales. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const MaterialsHandlers = ({
  ...
  setSelectedRow,
  setSelectedRows,
}: MaterialsHandlersProps): MaterialsHandlersReturn => {
  return {
