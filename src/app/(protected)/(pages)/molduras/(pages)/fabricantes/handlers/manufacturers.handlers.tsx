...
import {
  ...
  updateManufacturer,
} from "../actions/manufacturers.actions";
// Types
import type {
  ...
  SubmitHandlerProps,
} from "./types/manufacturers.handlers.types";
...
const submitHandlerCreate = async ({
  ...
  try {
    const { manufacturer, error, success } = await createManufacturer({
      values,
    });
    ...
    if (success && manufacturer) {
      setData((prev) =>
        [...prev, manufacturer].sort((a, b) => a.name.localeCompare(b.name)),
      );
      form.reset();
      setOpenDialog(false);
    ...
    console.error(error);
    toast.error("Error al crear el fabricante. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerEdit = async ({
  ...
  try {
    const { manufacturer, error, success } = await updateManufacturer({
      id: selectedRow.id,
      values,
    });
    ...
    if (success && manufacturer) {
      setData((prev) =>
        prev
          .map((item) => (item.id === manufacturer.id ? manufacturer : item))
    ...
    console.error(error);
    toast.error(
      "Error al actualizar el fabricante. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const submitHandlerDelete = async ({
  ...
  try {
    const { success, error } = await deleteManufacturer({ id: selectedRow.id });
    ...
    console.error(error);
    toast.error(
      "Error al eliminar el fabricante. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultipleManufacturers({
    ...
    console.error(error);
    toast.error(
      "Error al eliminar los fabricantes. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const ManufacturersHandlers = ({
  ...
  setSelectedRow,
  setSelectedRows,
}: ManufacturersHandlersProps): ManufacturersHandlersReturn => {
  return {
