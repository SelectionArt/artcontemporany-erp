...
import type {
  ...
  SubmitHandlerProps,
} from "./types/supports.handlers.types";
...
const submitHandlerCreate = async ({
  ...
  try {
    const { support, error, success } = await createSupport({ values });
    ...
    if (success && support) {
      setData((prev) =>
        [...prev, support].sort((a, b) => a.name.localeCompare(b.name)),
      );
      form.reset();
      setOpenDialog(false);
    ...
    console.error(error);
    toast.error("Error al crear el soporte. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerEdit = async ({
  ...
  try {
    const { support, error, success } = await updateSupport({
      id: selectedRow.id,
      values,
    });
    ...
    if (success && support) {
      setData((prev) =>
        prev
          .map((item) => (item.id === support.id ? support : item))
    ...
    console.error(error);
    toast.error(
      "Error al actualizar el soporte. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const submitHandlerDelete = async ({
  ...
  try {
    const { success, error } = await deleteSupport({ id: selectedRow.id });
    ...
    console.error(error);
    toast.error("Error al eliminar el soporte. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultipleSupports({
    ...
    console.error(error);
    toast.error(
      "Error al eliminar los soportes. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const SupportsHandlers = ({
  ...
  setSelectedRow,
  setSelectedRows,
}: SupportsHandlersProps): SupportsHandlersReturn => {
  return {
